const { getDataUsersById } = require("../query/userMangeController-db");
...
const getUserByEmail = async (connection, email) => {
  ...
  const generateTokens = (user) => {
    return {
      accessToken: generateToken(user, process.env.ACCESSTOKEN, "4h"),
      refreshToken: generateToken(user, process.env.REFRESHTOKEN),
      refreshTokenExp: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000),
    };
  };
  
  const manageUserSession = async (connection, userId, accessToken, refreshToken, refreshTokenExp, req) => {
    ...
    if (existingSession.length > 0) {
      await connection.execute(
        ...
        [refreshToken, accessToken, refreshTokenExp, req.headers["user-agent"], req.ip, existingSession[0].id]
      );
    } else {
      await connection.execute(
        ...
        [userId, refreshToken, accessToken, refreshTokenExp, req.headers["user-agent"], req.ip]
      );
    }
  };
  ...
  const logUserAction = async (connection, userId, req) => {
    await connection.execute(
      ...
      [userId, req.ip, req.headers["user-agent"], JSON.stringify({ timestamp: new Date().toISOString() })]
    );
  };
  ...
  const setSecureCookies = (res, accessToken, refreshToken, refreshTokenExp) => {
    ...
    res.cookie("accessToken", accessToken, { ...cookieOptions, expires: new Date(Date.now() + 60 * 60 * 1000) });
    res.cookie("refreshToken", refreshToken, { ...cookieOptions, expires: refreshTokenExp });
  };
  ...
  const checkEmailExists = async (connection, email, dataId = null) => {
    ...
    // Execute the query
    const [existingUsers] = await connection.execute(checkEmailQuery, queryParams);
  
    // If any user with the same email is found, throw an error
    if (existingUsers.length > 0) {
      throw new Error(dataId ? "Email already exists for another user" : "الابريد الالكتروني موجود مسسبقا");
    }
  };
  
  const updateUserData = async (connection, data) => {
    const { name, phone, ministries_id, entities_id, address_id, roleId, password, email, dataId } = data;
  ...
  const updatePermissions = async (connection, roleId, dataId) => {
    const [roleData] = await connection.execute(getDataRoleAndPermissionQuery, [roleId]);
    if (roleData.length === 0) {
    ...
    const editPermission = `
      ...
      WHERE user_id = ?
    `;
    await connection.execute(editPermission, [dataPermission.permission_id, dataId]);
  };
  ...
  const getDataWithPagination = async (connection, whereClause, queryParams, limit, offset) => {
    const query = `
  ...
  const checkActiveSession = async (connection, userId) => {
    const activeSessionQuery = "SELECT * FROM active_session_user WHERE user_id = ? AND is_active_session = true";
    const [activeSessions] = await connection.execute(activeSessionQuery, [userId]);
